...
class FakeEngine:
    ...
    async def deliver_message(self, message_name, *, trigger_id=None, correlation_id=None, payload=None):
        self.calls.append((message_name, trigger_id, correlation_id, payload))
        return self.result
...
async def test_delivered_returns_202(api):
    ac, engine = api
    r = await ac.post("/messages", json={"message_name": "rfi_reply", "trigger_id": "EXC-1", "payload": {"a": 1}})
...
async def test_no_match_is_404(api):
    ...
    r = await ac.post("/messages", json={"message_name": "x", "trigger_id": "EXC-1"})
...
async def test_invalid_payload_is_422(api):
    ...
    r = await ac.post("/messages", json={"message_name": "x", "trigger_id": "EXC-1", "payload": {}})
