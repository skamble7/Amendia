...
async def test_resolve_happy_path_dispatches_and_publishes():
    ...
    # one trigger_dispatched event published with the right routing key + fields
    assert len(pub.published) == 1
    event, routing_key, _ = pub.published[0]
    assert routing_key == "ingestor.trigger_dispatched.v1"
    assert event["trigger_id"] == "EXC-2026-000123"
    assert event["fetch_url"].endswith("/triggers/EXC-2026-000123")
...
async def test_reply_accepted_transitions_and_stores_instance_id():
    ...
    await svc.handle_reply(
        {"trigger_id": "EXC-2026-000123", "process_instance_id": "pi-1"},
...
async def test_reply_rejected_transitions_and_stores_reason():
    ...
    await svc.handle_reply(
        {"trigger_id": "EXC-2026-000123", "reason": "unknown_pack", "detail": "no such pack"},
...
async def test_accepted_reply_before_dispatch_is_ignored():
    ...
    await svc.handle_reply(
        {"trigger_id": "EXC-2026-000123", "process_instance_id": "pi-x"},
        "agent_runtime." + DISPATCH_ACCEPTED + ".v1",
    )
    assert (await repo.get("EXC-2026-000123")).status is IngestionStatus.RECEIVED
...
async def test_duplicate_accepted_reply_is_noop():
    ...
    await svc.handle_reply({"trigger_id": "EXC-2026-000123", "process_instance_id": "pi-1"}, key)
    await svc.handle_reply({"trigger_id": "EXC-2026-000123", "process_instance_id": "pi-1"}, key)
    rec = await repo.get("EXC-2026-000123")
    assert rec.status is IngestionStatus.ACCEPTED
