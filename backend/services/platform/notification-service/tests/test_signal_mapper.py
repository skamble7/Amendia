...
def test_event_type_parses_second_to_last_segment():
    ...
    assert event_type("ingestor.exception_dispatched.v1") == "exception_dispatched"
...
def test_hitl_created_signal_has_ids_only():
    ...
    assert sig == {
        "type": "hitl_task_created", "exception_id": "EXC-1",
...
def test_process_completed_carries_outcome():
    payload = {"process_instance_id": "pi-1", "exception_id": "EXC-1",
...
def test_unknown_event_is_ignored():
    assert to_signal({"exception_id": "EXC-1"}, _rk("agent_runtime", "some_other_event")) is None
    assert to_signal({"exception_id": "EXC-1"}, "malformed-key") is None
