# handlers.py
"""Deterministic (dumb) tool handlers for the ACH-exposure **Closeout** segment + the tool registry + the
...
ACTION_TOOLS = {"mark_completed", "purge_working_data", "notify_pega"}
...
# --------------------------------------------------------------------------- #
# Segment C — Closeout & Purge
# --------------------------------------------------------------------------- #

def verify_disposition(args: Dict[str, Any]) -> Dict[str, Any]:
    case = _case_id(args)
    instr = str(_dig(args, "instruction", "release") or "release").lower()
    disposition = "purged" if instr == "purge" else "released"
    return {"case_id": case, "applied": True, "disposition": disposition, "verified_at": _FIXED_TS}


def mark_completed(args: Dict[str, Any]) -> Dict[str, Any]:
    case = _case_id(args)
    return {"acknowledged": True, "action_id": action_id("mark_completed", case), "status": "performed",
            "performed_at": _FIXED_TS, "case_id": case, "state": "completed"}


def purge_working_data(args: Dict[str, Any]) -> Dict[str, Any]:
    case = _case_id(args)
    return {"acknowledged": True, "action_id": action_id("purge_working_data", case), "status": "performed",
            "performed_at": _FIXED_TS, "case_id": case, "purged": True, "artifacts_removed": 3}
...
TOOLS: List[Dict[str, Any]] = [
    _tool("verify_disposition", "Verify FIS actually applied the disposition (released or purged).",
          S.VERIFY_DISPOSITION_INPUT, S.VERIFY_DISPOSITION_OUTPUT, verify_disposition),
    _tool("mark_completed", "Mark the ACH exposure case state completed.",
          S.MARK_COMPLETED_INPUT, S.MARK_COMPLETED_OUTPUT, mark_completed),
    _tool("purge_working_data", "Purge the case's residual working data/artifacts.",
          S.PURGE_WORKING_DATA_INPUT, S.PURGE_WORKING_DATA_OUTPUT, purge_working_data),
    # Shared handback
