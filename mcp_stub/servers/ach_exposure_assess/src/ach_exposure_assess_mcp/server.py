# server.py
"""Streamable-HTTP MCP app exposing the ACH-exposure **Assess** segment tools.

Built on the official ``mcp`` SDK's low-level ``Server`` (so the hand-written schemas in ``schemas.py`` are the
exact ``inputSchema`` / ``outputSchema`` surfaced to ``tools/list``), served over streamable HTTP at ``/mcp``.
A ``/health`` route is provided for compose health-gating.
"""
...
SERVER_NAME = "ach-exposure-assess"
DEFAULT_PORT = "8075"
