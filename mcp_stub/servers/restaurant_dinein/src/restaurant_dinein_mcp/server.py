# server.py
"""Streamable-HTTP MCP app exposing the six restaurant dine-in capability tools.

Built on the official ``mcp`` SDK's low-level ``Server`` (so the hand-written schemas in
``schemas.py`` are the exact ``inputSchema`` / ``outputSchema`` surfaced to ``tools/list``), served
over **streamable HTTP** at ``/mcp`` — the transport the process-registry onboarding wizard
(ADR-025) and the agent-runtime both speak. A ``/health`` route is provided for compose
health-gating.

Each ``tools/call`` returns both a text content block and **structured content** (the tool's output
object), so the runtime's MCP client — which reads ``result.structuredContent`` — gets a schema-valid
artifact directly.
"""
...
SERVER_NAME = "restaurant-dinein"


def build_server() -> Server:
    """The low-level MCP ``Server`` with the six tools wired to their handlers."""
...
def create_app() -> Starlette:
    ...
    handshake required, plain-JSON responses) — friendly to both the wizard's MCP client and simple
    callers."""
    server = build_server()
...
def main() -> None:
    ...
    port = int(os.environ.get("MCP_PORT") or os.environ.get("PORT") or "8070")
