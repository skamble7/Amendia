...
"""ADR-047 D1: the ticket fetch-back (GET /tickets/{id}) must return the CLEAN domain trigger artifact — only
the ``art.dining.order_ticket`` fields — not the persisted row's store metadata. The pack's declared trigger
...
# The pack's declared trigger schema (art.dining.order_ticket) — additionalProperties:false over the 8 domain
# fields, mirroring what dispatch validates the fetched artifact against.
_ORDER_TICKET_SCHEMA = {
    ...
    "properties": {
        ...
        "tender": {"type": ["string", "null"]},
        "seated_at": {"type": ["string", "null"]},
    },
}

_ENVELOPE_FIELDS = set(_ORDER_TICKET_SCHEMA["properties"])
...
async def test_fetchback_returns_clean_domain_artifact_not_the_stored_row(client):
    ...
    gen = await client.post("/tickets/generate", json={"count": 1})
    assert gen.status_code == 201
    ticket_id = gen.json()["created"][0]["ticket"]["ticket_id"]

    resp = await client.get(f"/tickets/{ticket_id}")
    assert resp.status_code == 200
    payload = resp.json()

    # exactly the 8 domain fields — no store metadata rides along.
    ...
    Draft202012Validator(_ORDER_TICKET_SCHEMA).validate(payload)


async def test_fetchback_unknown_ticket_still_404s(client):
    assert (await client.get("/tickets/does-not-exist")).status_code == 404
