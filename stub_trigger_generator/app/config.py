...
All settings are read from the environment with the ``STUBTRIG_`` prefix (or a
...
class Settings(BaseSettings):
    # MongoDB — ADR-059: one domain-blind trigger-message store (was exceptions + tickets).
    MONGO_URI: str = "mongodb://localhost:27017"
    MONGO_DB: str = "amendia"
    MONGO_COLLECTION: str = "trigger_messages"
...
# Auth config (STUBTRIG_AUTH_ISSUER, _AUDIENCE, _JWKS_URI, _IDENTITY_BASE_URL,
# _INTERNAL_TOKEN, _COMPAT_STUB, ...).
auth_settings: AuthSettings = load_auth_settings("STUBTRIG_")
