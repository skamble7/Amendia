...
class MongoClient:
    """Thin wrapper owning the motor client + the trigger_messages collection."""
    ...
    async def ensure_indexes(self) -> None:
        ...
        # Unique on trigger_id → duplicate insert surfaces as HTTP 409 (idempotency).
    ...
    def collection_for(self, name: str) -> AsyncIOMotorCollection:
        """A sibling collection in the same db (e.g. a sibling collection)."""
        if self._db is None:
            raise RuntimeError("MongoClient not connected")
