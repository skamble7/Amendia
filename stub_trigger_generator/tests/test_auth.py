...
async def test_generate_unauthenticated_401(strict_client):
    r = await strict_client.post("/generators/wire/generate")
...
async def test_internal_token_fetchback_ok(strict_client):
    ...
    gen = await strict_client.post("/generators/wire/generate", headers={INTERNAL_HEADER: INTERNAL})
    assert gen.status_code == 201
    trig_id = gen.json()["created"][0]["trigger"]["trigger_id"]
    got = await strict_client.get(f"/triggers/{trig_id}", headers={INTERNAL_HEADER: INTERNAL})
