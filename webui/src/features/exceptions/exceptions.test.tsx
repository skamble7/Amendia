...
const ONE_INGESTION = [{
  exception_id: "exc-1", exception_type: "unable_to_apply", status: "received",
...
describe("Exceptions page — domain-neutral trigger panel (ADR-052)", () => {
  it("drives the trigger from the stub's generator catalog, not hardcoded reason codes", async () => {
    ...
    server.use(
      ...
      http.post(`${STUB}/tickets/generate`, async ({ request }) => {
        posted = { path: "/tickets/generate", body: await request.json() };
        return HttpResponse.json({ created: [{ ticket: { ticket_id: "TKT-9" } }] }, { status: 201 });
      }),
    );
    renderApp("/exceptions", "owner-1");
    ...
    expect(posted!.path).toBe("/tickets/generate");
