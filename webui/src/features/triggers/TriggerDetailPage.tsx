...
import { useTrigger, useIngestion } from "./queries";

export function TriggerDetailPage() {
  const { triggerId } = useParams();
  const { data: exc, isLoading, error } = useTrigger(triggerId);
  const { data: ingestion } = useIngestion(triggerId);
  ...
  if (!exc) {
    return <EmptyState title="Trigger not found" description="The id may be invalid." />;
  }
  ...
  return (
    <>
      <div className="mb-4">
        <Link to="/triggers" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="size-4" /> Triggers
        </Link>
      </div>
      <PageHeader
        title={exc.trigger_id}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_340px]">
        <div className="space-y-4">
          <Card>
            ...
            <CardContent><TriggerSummary exc={exc} /></CardContent>
          </Card>
                ...
                <ul className="space-y-1.5">
                  {attachments.map((a: any) => (
                    <li key={a.attachment_id}>
                      <a
                        href={attachmentUrl(exc.trigger_id, a.attachment_id)}
                        target="_blank"
                        rel="noreferrer"
          ...
          <Card>
            ...
            <CardContent>
              ...
              {ingestion?.status === "no_process" && (
                <p className="mt-3 text-sm text-danger">No active process pack matched this trigger.</p>
              )}
            </CardContent>
          </Card>
        </div>
