...
import type { StoredTrigger } from "@/api/types";

/** Extract the display payment summary from a StoredTrigger envelope. */
export function paymentSummary(exc: StoredTrigger | undefined | null) {
...
/** Compact trigger summary (used in the task context rail and elsewhere). */
export function TriggerSummary({ exc }: { exc: StoredTrigger | null | undefined }) {
  const s = paymentSummary(exc);
  const reasons = (exc as any)?.reason_codes ?? [];
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <IdMono value={exc?.trigger_id} className="text-sm" />
