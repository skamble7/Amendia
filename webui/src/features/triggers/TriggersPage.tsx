...
import { GenerateTriggerButton } from "./GenerateTriggerButton";
            ...
            <TableBody>
              {rows.map((ing) => (
                <TableRow
                  ...
                  tabIndex={0}
                  onClick={() => navigate(`/triggers/${ing.trigger_id}`)}
                  onKeyDown={(e) => e.key === "Enter" && navigate(`/triggers/${ing.trigger_id}`)}
                >
                  <TableCell><IdMono value={ing.trigger_id} className="text-foreground" /></TableCell>
                  <TableCell className="text-sm">{ing.trigger_type}</TableCell>
